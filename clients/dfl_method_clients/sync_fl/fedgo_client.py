...
class FedGOClient(Client):
    def __init__(self, client_id, dataset_index, full_dataset, hyperparam, device):
        ...
        input_shape = full_dataset[0][0].shape
        self.generator = Generator(self.noise_dim, input_shape).to(device)
        self.discriminator = Discriminator(input_shape).to(device)

        self.peer_models = {}
        self.peer_discriminators = {}
        self.is_later = False
    ...
    def send_model(self):
        ...
        disc_cpu = {k: v.cpu() for k, v in self.discriminator.state_dict().items()}
        return (self.id, {
            'model': model_cpu,
            'discriminator': disc_cpu
        })
    ...
    def _local_train_kd(self):
        ...
        with torch.no_grad():
            logits_sum = torch.zeros(fake_x.size(0), self.num_classes, device=self.device)
            weight_sum = torch.zeros_like(logits_sum)

            for cid, state_dict in self.peer_models.items():
                peer = type(self.model)().to(self.device)
                peer.load_state_dict(state_dict)
                peer.eval()

                disc = Discriminator(fake_x.shape[1:]).to(self.device)
                disc.load_state_dict(self.peer_discriminators[cid])
                disc.eval()

                out = peer(fake_x)
                d_out = torch.sigmoid(disc(fake_x))
                odds = d_out / (1 - d_out + 1e-6)

                logits_sum += odds * out
                weight_sum += odds

                del peer, disc, out, d_out, odds
                torch.cuda.empty_cache()

            pseudo_labels = logits_sum / (weight_sum + 1e-6)
